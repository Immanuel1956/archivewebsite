...
function init() {
  ...
  const crtMat = new THREE.ShaderMaterial({
    ...
    transparent: false,
    /* the 3DS's screen sits in a recessed well, behind the bezel's own front
       face at almost the same depth — depth-testing against that face was
       hiding the screen entirely on some frames, so it just always draws on
       top (renderOrder below pushes it after everything else, too). Some
       imported screen meshes also have their winding backwards relative to
       the camera, which face-culls the whole thing under the default
       FrontSide — DoubleSide sidesteps that regardless of which way it goes. */
    depthTest: false,
  ...
  /* No .glb — the shell is built here, procedurally: a generic handheld
     silhouette (nobody's real product), same button/screen naming the
     rest of this file already keys off of. Deferred to the end of init()
     (see the `setup(buildConsole())` call by the render loop) because
     `setup` reaches for `current`, `powerT` and friends, which are declared
     further down this function and aren't initialised yet up here. */
  function buildConsole() {
    const g = new THREE.Group();
    const bodyMat = new THREE.MeshStandardMaterial({ color: 0x141414, metalness: 0.42, roughness: 0.48 });

    const body = new THREE.Mesh(new RoundedBoxGeometry(16, 9, 2, 4, 0.9), bodyMat);
    g.add(body);

    const screen = new THREE.Mesh(new THREE.PlaneGeometry(10.4, 5.6), new THREE.MeshStandardMaterial({ color: 0x05070a }));
    screen.name = 'Screen';
    screen.position.set(0, 1.9, 1.011);
    g.add(screen);

    const dpad = [
      ['Button1', -4.4, -0.9],   // up
      ['Button2', -3.7, -1.6],   // right
      ['Button3', -4.4, -2.3],   // down
      ['Button4', -5.1, -1.6]    // left
    ];
    dpad.forEach(([name, x, y]) => {
      const m = new THREE.Mesh(new RoundedBoxGeometry(0.9, 0.9, 0.4, 2, 0.14), bodyMat.clone());
      m.name = name;
      m.position.set(x, y, 1.02);
      g.add(m);
    });

    const faceColors = [0x7fe3ff, 0xc8ff3c, 0xff2e7e, 0xffd166];
    const face = [
      ['Button5', 0, 0.7],    // triangle-slot
      ['Button6', 0.7, 0],    // circle-slot
      ['Button7', 0, -0.7],   // cross-slot
      ['Button8', -0.7, 0]    // square-slot
    ];
    face.forEach(([name, x, y], i) => {
      const m = new THREE.Mesh(new THREE.CylinderGeometry(0.42, 0.42, 0.4, 24), new THREE.MeshStandardMaterial({ color: faceColors[i], metalness: 0.3, roughness: 0.4 }));
      m.name = name;
      m.rotation.x = Math.PI / 2;
      m.position.set(4.4 + x, -1.6 + y, 1.02);
      g.add(m);
    });

    /* the bottom-edge row: home / select / start (plus four decorative aux
       buttons) — their average position is how the rest of this file works
       out which way is "down" on the shell */
    const row = [
      ['Button9', 0, -3.6],    // home
      ['Button10', -3.0, -3.6],
      ['Button11', -1.8, -3.6],
      ['Button12', 1.8, -3.6],
      ['Button13', 3.0, -3.6],
      ['Button14', -0.9, -3.6], // select
      ['Button15', 0.9, -3.6]   // start
    ];
    row.forEach(([name, x, y]) => {
      const m = new THREE.Mesh(new RoundedBoxGeometry(0.5, 0.32, 0.3, 2, 0.1), bodyMat.clone());
      m.name = name;
      m.position.set(x, y, 1.02);
      g.add(m);
    });
  ...
  function setup(src) {
    ...
    src.traverse((o) => {
      ...
      /* the glossy glass overlay sits right in front of the screen and washes
         out the live canvas underneath with its own reflections — the CRT
         shader already draws its own glass highlight, so the literal glass
         mesh is redundant and actively hurts here. */
      if (n.indexOf('glass') === 0) { bin.push(o); return; }
      /* the 3DS ships two screens plus their bezel-border trim, all named
         with a "screen" prefix — the live gallery goes on the top one only,
         so border/bottom variants are explicitly excluded. */
      if (n.indexOf('screen') === 0 && n.indexOf('border') === -1 && n.indexOf('bottom') === -1) screenMesh = o;
      const m = n.match(/^button(\d+)/);
      if (m) parts['b' + m[1]] = o;
      if (o.material) {
        o.material = o.material.clone();
        /* the swappable coloured shell plates — force them black regardless
           of the model's own texture/colour */
        const isShellPlate = n.indexOf('outer') !== -1 || n.indexOf('inner') !== -1 || n.indexOf('bits') !== -1 || n.indexOf('buttons') !== -1;
        if (isShellPlate) {
          o.material.map = null;
          o.material.color.setHex(0x0a0a0a);
          /* matte, not metallic — with no environment map to reflect, a
             high-metalness surface has almost no diffuse response and reads
             as flat dark navy under the invert lighting rig no matter what
             colour it's set to. Low metalness keeps it responding to the
             lights like plastic, so black stays black and inverted stays
             legibly white. */
        ...
        if (!isShellPlate && o.material.metalness !== undefined) {
  ...
  /* clickable siblings of the keyboard hints — step through the 9 works */
  const workPrevBtn = document.getElementById('workPrev');
  const workNextBtn = document.getElementById('workNext');
  if (workPrevBtn) workPrevBtn.addEventListener('click', () => press('left'));
  if (workNextBtn) workNextBtn.addEventListener('click', () => press('right'));
  ...
  new GLTFLoader().load(
    'assets/models/3ds.glb',
    (gltf) => { setup(gltf.scene); },
    undefined,
    (err) => {
      console.error('[psp] model failed, falling back to the built-in shell', err);
      setup(buildConsole());
    }
  );
  tick();
