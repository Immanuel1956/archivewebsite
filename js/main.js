...
(function () {
  ...
  /* always land on the hero, not wherever the browser last scrolled to */
  ...
  /* view counter — one real, shared count via a free public hit-counter API,
     keyed to this site's hostname so it survives across visitors */
  var viewsEl = document.getElementById('viewCount');
  if (viewsEl) {
    var ns = 'archivecrute-' + location.hostname.replace(/[^a-z0-9]/gi, '');
    fetch('https://api.countapi.xyz/hit/' + ns + '/views')
      .then(function (r) { return r.json(); })
      .then(function (d) { viewsEl.textContent = d.value.toLocaleString() + ' views'; })
      .catch(function () { viewsEl.textContent = '— views'; });
  }

  /* volume slider — drags VOL (the target the fade-in aims for) and, once
     audio is actually audible, the live element itself */
  var volSlider = document.getElementById('volumeSlider');
  if (volSlider && audio) {
    try {
      var savedVol = sessionStorage.getItem('sk_vol');
      if (savedVol !== null) volSlider.value = savedVol;
    } catch (e) {}
    VOL = Math.max(0, Math.min(1, volSlider.value / 100));
    volSlider.addEventListener('input', function () {
      VOL = Math.max(0, Math.min(1, volSlider.value / 100));
      if (audible) audio.volume = VOL;
      try { sessionStorage.setItem('sk_vol', volSlider.value); } catch (e) {}
    });
  }
